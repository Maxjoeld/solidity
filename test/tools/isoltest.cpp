...
void SyntaxTestTool::printContract() const
{
	if (m_formatted)
	{
		std::string const& source = m_test->source();
		...
		for (auto const& error: m_test->errorList())
			...
			{
				solAssert(static_cast<size_t>(error.locationStart) < source.length(), "");
				solAssert(static_cast<size_t>(error.locationEnd) < source.length(), "");
